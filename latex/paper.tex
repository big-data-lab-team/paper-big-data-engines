...
This work studies how performance benchmarks generalize across Big Data engines in an HPC environment.
...
Our system uses similar hardware as Compute Canada's B\'eluga cluster --- a representative cluster available to researchers in Canada --- and replicates as
close as possible its configuration.
This lets us compare Spark and Dask in the condition of a realistic
HPC system.
...
Due to resources limitation on our infrastructure, the Kmeans experiments
...
Our Kmeans resutls from Figure~{\ref{fig:kmeans_worker}} show a significant difference between Dask and Spark.
...
Moreover, Spark read all the data before starting computation while was still reading data alongside doing computation.
...
In this study, we found that Dask has a larger startup overhead than Spark; however, we did not in our previous study~\cite{8943502}.
