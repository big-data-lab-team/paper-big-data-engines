...
tasks that do not fit in the Array, DataFrame or Bag frameworks. 
...
brain. The original data is stored in 125 blocks in the MINC~\cite{minc}
HDF5-based format, available at
\url{ftp://bigbrain.loris.ca/BigBrainRelease.2015/3D_Blocks/40um} at the
...
pipeline (see Algorithm~\ref{alg:incrementation}). It reads blocks from the BigBrain
...
\textbf{Number of blocks:} Evaluating the frameworks on a different amount of blocks
