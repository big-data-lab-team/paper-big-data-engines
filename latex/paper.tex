...
\HL{
	...
	Similarly, we configured Spark to have eight executors per node, each with eight threads.
...
We observe that Spark reports three times more processes than Dask threads.
\HL{
...
Our Kmeans results from Figure~{\ref{fig:kmeans_worker}} show a significant difference between Dask and Spark.
...
This is unexpected, given that Dask and Spark are configured similarly and are allocated the same amount of resources.
...
\iffalse
The differences in performance are minimal between both engines. However, 
we added a “Recommendation” subsection to the discussion for qualitative 
difference and some caveats to avoid when using the engines.
\fi
\HL{
	The results from our experiments show that the performance difference between
	Dask and Spark is minimal, in general. 
	...
	easier to implement neuroimaging application.
	...
	Dask dashboard provides more information than Spark ones, making
	the debugging of Dask application easier and faster.
	...
	would be equal candidate as a Big Data engine.
}

\HL{
	% TODO add caveats to avoid when using the engines.
}
...
Therefore, while reducing this bottleneck should improve the performance of 
...
\HL{
	...
	With preprocessing comprising the majority of compute time for neuroimaging
	workflow, we ommitted GPUs from this study.
