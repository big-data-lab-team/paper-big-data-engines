...
On the other hand, Spark's Java, Scala, R, and Python APIs efficiently parallelize pipelines with minimal performance loss, due to the Java Virtual Machine. \TG{you could add a few words to explain why the JVM is useful for parallelization ("thanks to the JVM that ...")}
...
sleep delay to study the effect of task duration. \TG{these parameters don't seem to match the names used in Table I, they should be harmonized}
...
The bars are broken down into Idle, Read, Write, and other application-specific functions. \TG{Add a sentence to explain what Idle is. Is it exactly the overhead mentioned previously? if
so, I would just call it Overhead everywhere (in the figures and in the text)}
...
Figure \ref{fig:increment_itr} also shows that the total computation time increases as the total I/O time decreases.
This is explained by an I/O bottleneck on the Lustre file system.
When threads are busy with computations, they reduce the contention on Lustre, thus improving I/O speed for individual accesses \TG{but then read or write time should decrease, not computing time.}.
\begin{figure}[!t]
	\centering
...
% However, the Spark worker did not restart with a lower process count and no longer recomputed tasks \TG{removed this as it is discussed later more clearly}.
...
\TG{Could you add information about how the differences in total times translated into makespans, for each application? Either as sentences in the text or as an extra table.}
