...
Dask was designed with data locality and in-memory in mind to mitigate the data transfer bottleneck in Big Data workflows.
...
The Spark standalone is a simple built-in scheduler.
...
In the Spark Standalone scheduler, a \textit{leader} coordinates the resource provisioned by \textit{workers} in the cluster.
...
Python is a standard programming language in the scientific community.
This is particularly true in the neuroimaging field, with numerous specialized Python libraries to operate on data.
While serialization from Python to Java creates overhead, we found it to be minimal \cite{8943502}.
