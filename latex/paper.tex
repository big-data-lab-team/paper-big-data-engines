...
framework. It provides in-memory computing, data locality, and lazy
...
scheduler, Spark standalone, although YARN~\cite{yarn} and
Mesos~\cite{mesos} are also available. In the Spark standalone scheduler, a
...
Spark's and Dask's schedulers.} Dask has five main APIs: Dask Bags, Dask
Arrays, Dask Futures, Dask Delayed and DataFrames. We compared them all
except DataFrames as they are not adapted to the type of data we
considered. All APIs provide in-memory computing, data locality, and lazy
...
\href{https://docs.dask.org/en/latest/bag.html}{Dask Bags}
are parallel collections of Python objects, similar to Spark's RDD. Dask
Bags also offers optimized algorithms \TG{``optimized algorithms'' is
inaccurate: you should explain what they do} that mimic most of the
toolz~\cite{toolz} library. \href{https://docs.dask.org/en/latest/array.html}{Dask
Arrays} are used for the
processing of large arrays. They provide a parallel clone of the popular
NumPy library. \href{https://docs.dask.org/en/latest/delayed.html}{Dask
Delayed} supports arbitrary
tasks that does not fit in the Array, DataFrame or MapReduce \TG{Do you
mean Bags?} frameworks. Finally, \href{https://docs.dask.org/en/latest/futures.html}{Dask
Futures} is similar to Dask
Delayed as it supports arbitrary tasks, but it operates in real-time rather
than lazily.
