...
In figure~\ref{fig:histo_tt_block} 
%TODO
...
different block size. Note that the workers can process up to 64 tasks in paralle thus
...
Again, the overhead for 30 blocks is erroneous due a lower amount of task than thread
which result in higher overhead for thread based engines. Considering the overhead
error and that at 30 blocks the worker only use half their core, the  total time is
substantially the same for all block size. This is due lower block sizes having a
lower IO time, however, it is balanced by the resulting increase in overhead.
