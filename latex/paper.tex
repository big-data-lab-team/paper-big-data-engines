...
achieves fault tolerance through recording the data lineage, the sequence of 
operations required to produce the data. The RDD is the basis of Spark's other
data structures, namely, DataFrames
...
in the scientific Python ecosystem. Like Spark, Dask provides
in-memory computing, data locality, lazy evaluation, and fault tolerance by
means of data lineage. In contrast, Dask's core is purely in Python, eliminating
the cost of serialization to Java. Furthermore, it is lightweight and modular,
...
All Dask data structure APIs, with the exception of Dask Arrays and Dask
DataFrames, were used in our experiments.

While Python multithreading is limited by the
\href{https://docs.python.org/3/glossary.html#term-gil}{Global Interpreter Lock (GIL)}
as a result of CPython's non thread-safe memory management, Dask attempts to
bypass the GIL whenever possible and use multithreading to provide parallelism.
With the exception of Dask Bag, all APIs, by default, use the local multithreaded
scheduler. Dask Bag instead relies on the local multiprocessing scheduler. 

The Dask graph is the internal representation of Dask application to be executed 
by the scheduler. API Operations generate multiple
small tasks in the computation graph allowing an easier representation of complex
algorithms.
...
scheduler receives jobs from clients and assigns tasks to available workers. Similarly
to Spark's scheduler, Dask Distributed completes the processing of a graph branch
...
\subsection{Infrastructure}
 ...
 reserved for storage and is immune to eviction.
...
\subsubsection{Histogram}
 ...
 program, where the final result is obtained from all the input blocks (see
...
available on GitHub. This application can also be broken down into a map and
reduce phase. The map phase,
...
minus the start time of the application; the summation of those results is the total
overhead.
...
number of workers. Note that each worker has 8 CPUs. The makespan
...
For 30 blocks, Delayed and Futres have much higher overhead. This is because they are
...
shown. The computation time is significantly larger for Dask than for Spark. This is
potentially due to Python's GIL preventing Dask to parallelize the computation on
...
Figure~\ref{fig:histo_ms_block} shows the makespan for different block size. Again,
Spark is not compared at 30 blocks due to its \SI{2}{\giga\byte} task limitation.
Once more, Spark is significantly faster than Dask engines and Bag is slightly faster
...
increases the overhead time. Again, this is due to large overhead desynchronizing task
...
The reduce is done differently for the engines. Spark and Delayed compute all the
reduce tasks on one process (thread), however, Spark only computes it on the driver
process. On the other hand, Bag computes the reduce tasks on different workers and
sends the information to the driver process afterward.
...
in the number of workers. This is because the engines do not scale well. There is no
...
shown. The overhead and read time increase proportionally to the number of workers
thus increasing the total time. This is because (1) most workers read at the same
time due to a low computation-IO ratio and (2) there is more intra-worker and
...
different block size. Again, Spark is not compared at 30 blocks due to its
\SI{2}{\giga\byte} task limitation. Overall, there is no substantial difference for
...
higher that it really is. Considering the overhead error and that at 30 blocks the
workers only use half their core, the total time is substantially the same for all
block size. This is due to lower block sizes having a lower IO time, however, it
balances out by the resulting increase in overhead.
...
between a read and compute task or compute and read task. On the other hand, Delayed
overhead is dispersed between all types of tasks. This causes Delayed to overlap more
compute and read tasks simultaneously among the different workers.

In figure~\ref{fig:histo_np_gantt}, read tasks are initially similar for all engines.
However, when computation starts the subsequent read tasks are much shorter for
Delayed. This is due the desynchronization of the task reducing the IO bottleneck.
...
Figure~\ref{fig:bids_ms_worker} shows the makespan of the application when varying
the amount of workers. Overall, there is no substantial difference between the
engines. The makespan scales well with the increase in workers, however, the scaling
reduces considerably when reaching 8 workers. This is due to an increase in overhead
and of the IO requirement increasing with the number of workers --- more tasks are
read concurrently and the application becomes more data-intensive.
...
shows. We could use a parallel shared file system, like Lustre \MD{cite lustre if not
done already}, to reduce the impact of operations with the disk, however, we would
quickly be limited by the network's bandwidth.
