...
causes data-intensive applications to take a significant amount of time for data transfer. As we
...
data transfer times of parallel applications simultaneously writing to shared storage.
...
see that when a low number of data transfer tasks are overlapping, this diminishes the bandwidth
limitation from the NFS. We want to explore solutions to desynchronize data transfer tasks in
...
results in more data transfer compensated by lower overhead time and vice-versa.
...
size as the data transfer time should be reduced substantially. It is worth noting that the Dask
